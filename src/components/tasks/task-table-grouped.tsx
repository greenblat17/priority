...
export function TaskTableGrouped({
  ...
  // Keyboard navigation
  useTaskListNavigation(allTasks, selectedIndex, setSelectedIndex, (task) =>
  ...
  const groupedTasks = tasks.reduce(
    ...
    {} as Record<
      string,
      { group: TaskGroupType | null; tasks: TaskWithGroup[] }
    >
  )
  ...
  const renderTaskRow = (task: TaskWithAnalysis, index?: number) => {
    ...
    const taskIndex =
      index !== undefined ? index : allTasks.findIndex((t) => t.id === task.id)
        ...
        <TableCell className="w-[140px]">
          <div className="flex items-center gap-2">
            <ICEScoreBadge
              ...
              size="sm"
            />
            {task.analysis?.priority !== undefined &&
              task.analysis?.priority !== null && (
        ...
        <TableCell className="w-[80px] text-right">
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
              <Button
                variant="ghost"
                className="h-8 w-8 p-0 opacity-60 hover:opacity-100 group-hover:opacity-100 transition-opacity duration-200"
              >
