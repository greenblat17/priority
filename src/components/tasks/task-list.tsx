...
const GroupManagementDialog = dynamic(
  () =>
    import('@/components/tasks/group-management-dialog').then(
      (m) => m.GroupManagementDialog
    ),
  { ssr: false, loading: () => null }
)
const TaskKanbanView = dynamic(
  () => import('./task-kanban-view').then((m) => m.TaskKanbanView),
  { ssr: false, loading: () => null }
)
const DuplicateReviewDialog = dynamic(
  () =>
    import('./duplicate-review-dialog').then((m) => m.DuplicateReviewDialog),
  { ssr: false, loading: () => null }
)
...
export function TaskList() {
  ...
  const { filters, updateFilter, resetFilters, isInitialized } =
    useFilterPersistence()
  const [currentPage, setCurrentPage] = useState(1)
  const [showQuickAdd, setShowQuickAdd] = useState(false)
  const [showGroupDialog, setShowGroupDialog] = useState(false)
  const [activeView, setActiveView] = useState<'all' | 'active' | 'backlog'>(
    'all'
  )
  ...
  useEffect(() => {
    ...
    const handleOpenDuplicateReview = (event: CustomEvent) => {
      console.log(
        '[TaskList] Received openDuplicateReview event:',
        event.detail
      )
      if (event.detail) {
    ...
    window.addEventListener(
      'openDuplicateReview',
      handleOpenDuplicateReview as EventListener
    )
  ...
  const filteredAndSortedTasks = processedTasks
    ?.filter((task) => {
      ...
      if (
        ...
        task.analysis?.category !== filters.category
      )
        return false
      ...
      if (displaySettings.completedIssues === 'hide' && task.status === 'done')
        return false
      if (
        displaySettings.completedIssues === 'recent' &&
        task.status === 'done'
      ) {
    ...
    .sort((a, b) => {
      ...
      if (
        displaySettings.showCompletedByRecency &&
        a.status === 'done' &&
        b.status === 'done'
      ) {
        comparison =
      ...
      switch (displaySettings.ordering) {
        case 'priority':
          ...
          const aScore =
            a.analysis?.ice_score || (a.analysis?.priority || 0) * 10
          const bScore =
            b.analysis?.ice_score || (b.analysis?.priority || 0) * 10
  ...
  const taskCounts = {
    ...
    active:
      processedTasks?.filter((t) => ['todo', 'in_progress'].includes(t.status))
        .length || 0,
  ...
  return (
    <div className="space-y-6">
      ...
      <BulkActionsBar
        ...
        onDelete={() => {
          if (
            confirm(`Are you sure you want to delete ${selectedCount} tasks?`)
          ) {
      ...
      {duplicateReviewData && (
        <DuplicateReviewDialog
          ...
          potentialDuplicates={duplicateReviewData.similarities.map((sim) => {
            const task = duplicateReviewData.duplicates.find(
              (d) => d.id === sim.taskId
            )
            return {
              taskId: sim.taskId,
          ...
          onAction={async (action) => {
            if (
              action.action === 'create_and_group' &&
              'selectedTaskIds' in action
            ) {
              console.log('[GROUPING] Action received:', action)
              console.log(
                '[GROUPING] Selected task IDs:',
                action.selectedTaskIds
              )
